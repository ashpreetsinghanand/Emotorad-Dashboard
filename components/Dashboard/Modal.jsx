...
//   ref
  ...
  const handleDone = () => {
    ...
    const userData = { user,name, email, contact, instagram, yt };
  
    // Make an API request to insert data into the emotorad table
    fetch("/api/insertUserData", {
      ...
      .then((response) => {
        if (response.ok) {
          // Upon successful insertion, redirect to /users
          router.push("/users");
        } else {
          // Handle error cases
          alert("Failed to insert data into emotorad table");
        }
      })
      .catch((error) => {
        alert("Error:", error);
  ...
  return (
    ...
    onClick={handleOutsideClick}>
          ...
          <button className=" absolute top-0 right-0   rounded-full  mx-2 mt-4 text-[#e4e5e5] hover:text-queueTxt bg-[#f6f6f6]" onClick={closeShowModal}>
          <div size="20px"> X
